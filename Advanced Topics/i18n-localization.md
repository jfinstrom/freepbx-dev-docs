...
FreePBX supports multi-language admin UI through PHP gettext and JavaScript Jed. Module-specific translations live in per-module `i18n/` directories. Getting the domain right matters — use the wrong function and your strings won't translate.

## PHP Translation

### `_()` function

PHP `_()` is the **gettext extension** function — not defined by FreePBX. It activates when `View::setLanguage()` configures gettext:
...
- Don't hardcode user-visible strings without `_()` or `modgettext::_()`.