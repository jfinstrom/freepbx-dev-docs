...
FreePBX layers its error handling: a custom `freepbx_error_handler`, Whoops for exception rendering, and context-specific handlers depending on whether you're in CLI, web GUI, or AJAX. Understanding this chain helps you debug module issues and set the right handler for external scripts.
...
Bootstrap settings control the behavior:
...
Whoops picks a handler based on how PHP is running:
...
Output is controlled by `$amp_conf['PHP_ERROR_HANDLER_OUTPUT']`:
...
The 404 code triggers a hook cache rebuild retry in `Hooks.class.php`.
