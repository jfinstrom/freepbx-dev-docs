...
The admin dashboard notification system is how FreePBX surfaces alerts — config problems, security issues, update notices. Notifications live in the `notifications` DB table and are accessed via the BMO `Notifications` class or the legacy `notifications::create()` shim.
...
- Use consistent IDs so the delete-on-fix pattern works.
...
- Don't spam notifications on every page load — check `exists()` first.