---
title: Reload and Apply Config Flow
...
`fwconsole reload` is the heart of FreePBX config application. It replaced the deprecated `retrieve_conf` shell script. A reload regenerates Asterisk configs, symlinks module assets, runs dialplan hooks, and optionally reloads Asterisk. The GUI "Apply Config" button triggers the same pipeline.
...
Don't build new integrations against `retrieve_conf`.
...
- Reload requires a working AMI connection.