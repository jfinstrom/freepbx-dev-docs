---
title: fwconsole CLI — Module Console Commands
...
`fwconsole` is FreePBX's Symfony Console application for administration. Modules register commands via `module.xml` `<console>` section (preferred) or the legacy `Console/*.class.php` scan. The helloworld module has a working example.
...
If no `<console>` section exists but a `Console/` directory is present, `fwconsole` scans for `*.class.php` files and logs:
...
- A bad `include` in a Console class can break all `fwconsole` commands for that module.
