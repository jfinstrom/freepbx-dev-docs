...
FreePBX exposes an OAuth-scoped GraphQL API for PBX automation. Framework ships core providers; the `api` module provides base classes, routing, and auth. Modules add their own providers in `Api/Gql/`. The backup module is a solid reference implementation.
...
Scopes are set on the request via OAuth token. Check them in your callbacks:
...
- Guard every query/mutation callback with a scope check — unguarded callbacks expose operations.
...
- `restapi` module is deprecated — migrate to `api` module (PBX 16+).