...
For recurring module tasks, FreePBX has a centralized job registry in the `cron_jobs` DB table. Modules register command or class-based jobs; `fwconsole job --run` executes whatever's due. This is the preferred path over raw crontab manipulation for module-owned work.
...
If a job already exists (same modulename + jobname), this updates all fields **except** `enabled`.
...
Call `init()` from module `install()` to activate the job runner.
...
- Call `Job::init()` after registering jobs to install the crontab runner.
- Call `removeAllByModule()` in `uninstall()` to prevent orphaned jobs.
- Job runner uses a file lock — concurrent runs exit early.
- Max runtime default is 30s — increase for long-running tasks.