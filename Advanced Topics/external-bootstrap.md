---
title: External Bootstrap — Non-Module PHP Access
...
Not everything lives inside `admin/modules/`. AGI scripts, cron jobs, CLI utilities, and custom web endpoints all need FreePBX resources — database, config, Astman — without going through `config.php`. The entry point is `/etc/freepbx.conf`, which pulls in `bootstrap.php`.
...
Once bootstrapped, external scripts have the same resource access as internal modules.

## Entry Point: `/etc/freepbx.conf`

Installed by the framework installer:
...
**Deprecated:** `/etc/asterisk/freepbx.conf` — don't rely on it.
...
Resources skipped at bootstrap (Astman, module functions) cannot be initialized later in the same request.
...
Module class files load on first `FreePBX::Modulename` access via `Self_Helper::autoLoad()`.
...
- External web scripts should set `freepbx_auth = false` then implement their own auth
...
CLI scripts always get `FREEPBX_IS_AUTH`.
