...
FreePBX uses PHPUnit 4.5 for integration tests against a live install. Tests live in `utests/` directories. These are integration tests, not isolated unit tests — they need `/etc/freepbx.conf`, a working database, and often a live Asterisk AMI connection.
...
The `@backupGlobals disabled` annotation is required for PDO/FreePBX singleton tests.
...
- Don't commit tests that modify production data without cleanup.