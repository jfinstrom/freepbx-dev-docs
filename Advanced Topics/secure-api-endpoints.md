---
title: Secure API Endpoint Patterns
...
Building API endpoints for FreePBX means getting authorization, async safety, and remote-access hardening right. This covers the patterns used across GraphQL and REST — OAuth scopes, transaction polling, operation locking, and input validation.

**Dependency:** `api` module for OAuth and routing.

## OAuth Scope Authorization

All API operations require a valid OAuth token with appropriate scopes.
...
- REST unauthorized returns JSON body, not HTTP 401.
