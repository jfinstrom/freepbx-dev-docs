...
The global admin search bar and per-module content search are powered by the framework `Search` BMO class. Modules opt in by implementing a `search()` method. The AJAX entry point routes through `ajax.php` with `module=search`.
...
- Search AJAX does not require a separate module — uses framework `Search` class.