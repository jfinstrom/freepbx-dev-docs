---
title: Module Lifecycle — Install, Upgrade, Uninstall
...
FreePBX modules move through install, upgrade, enable/disable, and uninstall states — usually driven by `fwconsole ma` but ultimately governed by `module.xml` and your BMO class hooks. This doc walks through what the framework does at each transition so you know where to put your setup and teardown logic.
...
The `modules` table tracks where each module sits. The legacy `module_functions` class does the heavy lifting, exposed through `FreePBX\Modules`.
...
The `BMO` interface documents an optional `upgrade()` method, but the framework never calls it — `_runscripts()` only invokes `install()`. Put upgrade logic in `install()` with version checks, or use a procedural `upgrade.php` in your module directory.
...
Each section in `module.xml` kicks in at a specific lifecycle phase:
...
Know what happens when things go wrong — install and uninstall behave asymmetrically on exceptions.
