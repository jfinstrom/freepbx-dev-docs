---
title: GUI Page Routing, showPage, and Views
...
FreePBX admin pages flow through `config.php` — it resolves your menu item, runs form processing hooks, includes your `page.*.php` file, and assembles the footer with action buttons. Your BMO class's `showPage()` and `load_view()` templates handle the actual HTML. Get the order right (mutations before render) and pages feel natural; get it wrong and you'll wonder why your form submit did nothing.
...
These query params drive routing and form state across most modules.
