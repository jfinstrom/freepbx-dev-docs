---
title: Bootstrap Chain and Global State
...
Every FreePBX web request and CLI command starts the same way: `/etc/freepbx.conf` pulls in `framework/amp_conf/htdocs/admin/bootstrap.php`, which wires up autoloading, the database, Asterisk Manager, and the BMO singleton. If you're debugging "why isn't my module loaded?" or writing a standalone script, this is the chain you need to understand.
...
Set these in `/etc/freepbx.conf` *before* including bootstrap to control what gets initialized. Handy when you're writing a cron script that doesn't need the full GUI stack.
...
These globals are still everywhere in legacy code. BMO modules should prefer the accessor patterns shown below, but you'll bump into the globals constantly when reading older modules.
...
Key settings you'll reference often:
...
After bootstrap completes, these flags tell you what actually succeeded:
...
Whoops handler varies by context — set `whoops_handler` in `$bootstrap_settings` if you need something specific.
...
## Legacy global patterns you'll still see
