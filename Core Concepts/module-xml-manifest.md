---
title: module.xml Manifest Structure
...
Every FreePBX module ships a `module.xml` — it's the manifest that tells the framework who you are, what you depend on, which GUI pages you register, and how your database schema looks. The `FreePBX\Modules` class parses it via `getXML()`, `checkConflicts()`, `getInfo()`, and `getActiveModules()`.
...
These elements describe your module to the admin UI and the module admin system. Most are display-only, but `<rawname>`, `<version>`, and `<category>` affect install and routing.
...
Pick the category that matches where your module shows up in Module Admin.
