...
BMO modules talk to MySQL through `FreePBX\Database`, which extends `\PDO`. You'll get a reference to it in your constructor and use standard prepared statements from there. The old `global $db` wrapper still exists in legacy code — don't use it in new modules.
...
Pick the fetch mode that matches how you want to work with the result set.
