---
title: Request_Helper — $_REQUEST Processing
...
Stop reading `$_REQUEST` directly in BMO modules. `Request_Helper` (in `libraries/BMO/Request_Helper.class.php`) gives you sanitized access via `getReq()`, bulk import to KVStore via `importRequest()`, and an override layer via `setReq()`. Every BMO module inherits these methods through `FreePBX_Helpers`.
...
Sanitized single-variable read from `$_REQUEST`. This is your default for form handling in `doConfigPageInit()`.
...
The `$def` parameter controls what you get back when the key isn't set — pay attention to the difference between `false`, `true`, and an explicit default.
...
Set sensible defaults so you don't repeat the same fallback logic everywhere.
