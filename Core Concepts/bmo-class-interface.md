---
title: BMO Class Interface — Modulename.class.php
...
Your `{Classname}.class.php` is the main artifact for a FreePBX 12+ module — it handles install/uninstall, GUI pages, AJAX, dialplan hooks, and config generation. The helloworld module is the canonical reference; this doc maps out every method the framework may call on your class.

## File Convention

The filename and class name derive from your `rawname` in `module.xml`.
...
## `doConfigPageInit` — Form Processing Before Render

Handle mutations here, not in `showPage()`. The `$page` argument is the current display name (e.g., `helloworld`).
...
## `getActionBar` — Bottom Form Buttons

Return an empty array when no buttons are needed. Gate on `$request['display']` matching your menuitem key.
...
## `showPage` — Page Content Rendering
...
## AJAX Authorization and Handling

Authorize in `ajaxRequest()`, execute in `ajaxHandler()`.
...
Tweak these in `ajaxRequest()` to change how the AJAX handler treats your command.
